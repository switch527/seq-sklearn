"""C-MAPSS FD003 turbofan engine degradation dataset (NASA, OPEN).

100 engines under a single operating condition, two fault modes
(HPC degradation + fan degradation). Harder than FD001 because the
model has to discriminate fault types implicit in the sensor
signature. Same RUL framing as FD001.
"""
...
_SPEC = DatasetSpec(
    ...
    observation_cutoff_rule=(
        ...
        "FD003 carries two fault modes (HPC + fan) under a single "
        "op condition; the model must discriminate the implicit "
        "fault type from sensor signatures alone."
    ),
...
def load(cache_root: Path) -> PanelDataset:
    ...
    with zipfile.ZipFile(archive) as zf:
        try:
            text = zf.read("train_FD003.txt").decode("utf-8")
        except KeyError as exc:
            raise DatasetIOError(
                f"{_NAME}: archive {_SPEC.archive_basename!r} does not contain 'train_FD003.txt'"
            ) from exc
