...
|---|---|---|---|
| R-B38-Risk-1 | Float-order drift between sum-of-sums (fast) and sum-of-rows (naive) breaks equivalence. | Low | `pytest.approx` with reasonable tolerance (e.g., abs=1e-9). Bootstrap CIs are inherently noisy; ULP-level differences are fine. |
