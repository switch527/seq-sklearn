...
Q12 (mkdocs vs. sphinx) is RESOLVED to Sphinx (user-ratified at
Phase 12 design-review R1, superseding the earlier interim mkdocs
resolution); A12 here documents the Sphinx implementation specifics.
...
docs/
  ...
  reference/                        Diátaxis reference tier (api.md, config.md, glossary.md, observability.md, determinism.md); autosummary stubs land under reference/api/
  explanation/                      Diátaxis explanation tier (design rationale)
  about/                            release-engineering surface (versioning, release_checklist, migration_template, changelog, contributing, installation)
  design/                           internal governance: hosts requirements/architecture/implementation_plan/phase plans via toctree with "internal governance, not user docs" callout
  examples/                         sphinx-gallery (executed .py + README.rst)
  research/*.md                     13 grounded research briefs (read-only after consensus; excluded from the docs build)
  references/                       reference PDFs, repo artifacts (excluded from the docs build)
```
...
Sphinx Theme, hosted on Read the Docs.** Resolved from requirements
Q12/Q16 (the earlier interim mkdocs resolution was overturned at
Phase 12 design-review R1: the sklearn / time-series-ML peer cluster
- scikit-learn, sktime, aeon, skorch, tslearn, darts - is
near-unanimously Sphinx, so matching it is a credibility signal to
exactly the target audience; sphinx-gallery closes the executable
examples-gallery gap; `intersphinx` gives free cross-references into
sklearn / numpy / torch / pydantic). Stack rationale:
`docs/readme_and_docs_plan.md` (now the ratified stack-decision
doc).
...
docs = [
    ...
    "numpydoc>=1.8,<2",
    "myst-parser>=4.0,<5",            # Markdown sources alongside reST
    "sphinx-copybutton>=0.5,<0.6",    # one-click copy on every snippet
...
constraint / `Field(description=...)` / validators, respecting the
`TFTConfig <- BaseModelConfig <- BaseTrainingConfig` inheritance via
`:inherited-members:`. Plain `autodoc` renders fields as bare
attributes and surfaces neither validators nor field metadata.

**Site layout** (`docs/`, the existing `docs/conf.py` skeleton is the
foundation; Diátaxis nav, see the Phase 12 plan):

```
docs/
  index.md                 value prop + above-the-fold quickstart
  tutorial/                one linear finishable spine
  how-to/                  task-named guides (incl. panel_data,
                           calibration, optuna, onnx, persistence)
  reference/               autosummary API + autodoc-pydantic config
                           tables + glossary + observability (F11)
  explanation/             the "why" docs (incl. not-a-forecaster)
  about/                   changelog, versioning/deprecation policy,
                           migration template, release checklist
  design/                  requirements.md, architecture.md,
                           research/*, phase plans hosted as-is
  examples/                sphinx-gallery, executed at build time
```

`sphinx-build -W --keep-going` (warnings-as-errors) is the CI gate:
it fails on broken cross-references, unknown autosummary targets,
nav/toctree omissions, and dead links (via `sphinx.ext.linkcheck`
run separately on a non-blocking schedule). This replaces the
`pr.yml` no-op docs job.

**Example execution gate.** `sphinx-gallery` executes every
`docs/examples/*.py` at build time and captures stdout/plots; a
build failure on any example is the executable-snippet gate.
Docstring snippets are gated by `pytest --doctest-modules
src/seq_sklearn/` (requirements N1); reST/MyST prose snippets use
`.. testcode::` / `.. doctest::` run by `sphinx.ext.doctest` in the
same CI job.

**numpydoc + autosummary recipe**:
...
numpydoc_show_class_members = False  # autosummary owns member listing
...
docs:            sphinx-build -W (html + doctest)         (~40s)
snapshot-guard:  scripts/check_snapshots.sh               (~5s)
```

Wall-clock equals `max(10, 30, 120, 60, 40, 5) = 120s` (~2 minutes),
dominated by `test-unit`. Well under the 5-minute N2 budget. The
docs build (HTML + the `-b doctest` pass + sphinx-gallery executing
the small curated example set on CPU with `max_epochs=1`) stays in
the tens of seconds; the Phase 12 plan pins a
`DOCS_DOCTEST_BUDGET_S` regression guard.
...
3. **Native ONNX `Attention` op (opset 23)**: PyTorch issue #149662
   ...
   ONNX-export path.
4. **`autodoc-pydantic` rendering of `Mapping[str, int]`**: the
   ...
   structured. (Was a `griffe-pydantic` check; reconciled to the
   Sphinx stack, Phase 12 R1.)
...
- **`autodoc-pydantic` `Mapping[str, int]` rendering check
  (arch r1-NITPICK 2; reconciled to Sphinx, Phase 12 R1).** Easier
  to verify in 30 seconds at implementation time than to research
  now; kept in A20 item 4 for the implementation phase.
