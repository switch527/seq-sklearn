...
| 9 | All N1-N7 requirements are met | N1-N6 are CI-gated on every PR (see items 1-3 above). N7 absolute budgets are validated by `tests/perf/test_n7_absolute.py` (marked `gpu` and `slow`, NOT in PR CI). **Release-checklist step:** run `pytest -m "gpu and slow" tests/perf/test_n7_absolute.py` once on an A100/T4/4090 and record the four N7 numbers in the `CHANGELOG.md` v1.0.0 entry. |
