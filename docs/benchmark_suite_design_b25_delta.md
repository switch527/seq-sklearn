...
that report has a non-None `per_fold_cis: list[FoldCI]`. The
footnote lists, for each fold in each row, the rollup-row
identifier columns followed by `fold`, `metric_mean`,
`metric_ci_lo`, `metric_ci_hi`, `ci_method`,
`ci_fallback_reason`. Rows with `per_fold_cis is None` are
omitted. The same trigger rule and helper apply to all 5
renderers (raw_loss, pairwise, training_time, hpo_uplift,
ensemble_lift) for parity. Mirrors the B24 BCa health
footnote shape.
...
def render_per_fold_cis_footnote(
    ...
    *,
    ...
    Caller pre-filters: pass only rows whose `per_fold_cis`
    is not None AND not empty. Empty input returns "".

    Each row must carry `per_fold_cis: list[FoldCI]` where
    each FoldCI carries `fold_index, n_seeds, n_entities,
    metric_mean (nullable), metric_ci_lo (nullable),
    metric_ci_hi (nullable), ci_method, ci_fallback_reason
    (nullable)`. All 5 v1 RollupRow classes carry the
    `per_fold_cis` field; a future row type lacking it would
    silently render with no fold rows via `getattr(row,
    "per_fold_cis", []) or []`.

    Rows in `rollup_with_per_fold` are sorted by
    `group_columns[0]`; folds within each row are emitted in
    `fold_index` ascending order (already enforced by
    ...
    renders as `-`. The 120-char truncation matches
    `render_bca_health_footnote` and
...
|---|---|
| `raw_loss.py` | the B24 `render_bca_health_footnote` block (added in B24 at the post-`render_rollup_skipped_footnote` site at `:452`) |
| `ensemble.py` (pairwise) | the B24 block at the post-`render_rollup_skipped_footnote` site at `:375` |
| `training_time.py` | the B24 block at the post-`render_rollup_skipped_footnote` site at `:382` |
| `hpo_uplift.py` | the B24 block at the post-`render_rollup_skipped_footnote` site at `:747` |
| `ensemble_lift.py` | the B24 block at the post-`render_rollup_skipped_footnote` site at `:462` |
...
3. Wilcoxon / Friedman blocks (where applicable)
4. Partial-coverage footnote (B16)
5. Oracle partial-coverage footnote (B23 / B24 columns)
6. Rollup-skipped footnote (B14 / B16)
7. **Bootstrap CI method footnote (B24)**
8. **Per-fold CIs footnote (B25, NEW)**

The per-fold section is last because per-fold data is the
most granular and least relevant to a top-level reader.
...
7. `test_per_fold_cis_footnote_float_format_4_decimals`:
   ...
   appears in the rendered row (banker rounding;
   `%.4f` convention).
...
9. `test_per_fold_cis_footnote_emits_fold_rows_in_ascending_fold_index_order`:
   one rollup row with three FoldCI entries in reverse
   `fold_index` order (2, 1, 0); compute_per_fold_cis
   guarantees ascending sort, but the helper must not
   re-shuffle. Construct the FoldCI list pre-sorted (since
   it is the production contract) and assert the rendered
   row order is fold 0, fold 1, fold 2.
10. `test_per_fold_cis_footnote_cell_data_reads_correct_fields`:
    FoldCI with specific values for all 8 fields; assert all
    values appear in the rendered output. Kills a
    getattr-wrong-attr bug.
...
### B25.3.6 Byte-pin regression

23. The B17 byte-pin renderer tests must continue to pass.
    The B17 fixtures all default `per_fold_cis=None`
    (verified via grep at
    `tests/benchmarks/test_b17_byte_identity_pins.py`) so
    the new Per-fold CIs footnote does NOT fire. The B17
    absent-substring assertion at line 166 / 298 / 382
    `assert "per_fold_cis" not in md` tests the raw FIELD
    NAME not surfacing; the new section heading is
    `"### Per-fold CIs"` (with the space), so the
    underscore-name assertion still passes. No B17 test edit
    needed for B25.

### B25.3.7 Expected test delta

Baseline (post-B24 main `6ef4fcd`): 973 tests collected.

- Existing tests: 973 → 973 unchanged.
- B25-new: 22 named tests + 4 extra parametrize collected
  on test #21 (5 renderers - 1 named = 4 extras).
- Total named: 973 + 22 = 995.
- Total collected: 973 + 22 + 4 = 999.
